...
module tb_edison_top();
    ...
    parameter LONG_GAP_LIM = 50;
    parameter SPACE_LIM = 140; /* Tick generator: CLK_FREQ / 100 = 1 tick = increment counter
    DOT_MAX = 10, GAP_MAX = 30, SPACE_MAX = 50
    Counter frequency: increment every 10 cycles => CLK_FREQ = 1000
    => To increment module counter, send 1000 cycles of light_on
    Send Dot: Below 10000 cycles
    Send Dash: Above 10000 cycles
    Send Short Gap: Below 30000 cycles
    Send Lop Gap: Below 30000 cycles
    Send Space: Above 50000 cycles */
    ...
    edison_top #(BIT_WIDTH, 4, 8, HOLD_MAX, 16, 1_000_000, DOT_LIM, LONG_GAP_LIM, SPACE_LIM) u_edison (.*);

    initial clk = 0;
    always #(CLK_PERIOD / 2) clk = ~clk;

    /*logic [BIT_WIDTH-1:0] target_comparator;
    always simulate_adc_cycle(target_comparator);*/
    ...
    task send_pulse(input int num_ticks);
        ...
        while(u_edison.u_decoder.current_state != 3'b001) simulate_adc_cycle(12'd2500);
        ...
        while(32'(u_edison.u_decoder.mark_counter) < num_ticks) begin
            simulate_adc_cycle(12'd2500); // 2500 is "LIGHT ON" (> 1000 baseline)
            // target_comparator = 12'd2500;
            $display("[DECODER] Mark Counter %0d", u_edison.u_decoder.mark_counter);
        end
    endtask
    ...
    task send_gap(input int num_ticks);
        ...
        while(u_edison.u_decoder.current_state != 3'b011) simulate_adc_cycle(12'd900);
    ...
    task state_debug();
        $display("[DECODER] Current State %0d and Index %0d", u_edison.u_decoder.current_state, u_edison.u_decoder.tree_index);
    endtask

    initial begin
        nrst = 0; pb = '0; rxdata = '0; rxready = 0; u_edison.char_ready = 0;
        ...
        repeat(50) @(posedge clk); screen_display();
        ...
        repeat(50) @(posedge clk); screen_display();
        repeat(SPACE_LIM) @(posedge clk); screen_display(); // Blankspace (long inactivity)
        send_pulse(DOT_LIM); send_gap(SHORT_GAP_LIM); send_pulse(DOT_LIM); send_gap(SHORT_GAP_LIM); send_pulse(DOT_LIM); send_gap(LONG_GAP_LIM); // S
        repeat(50) @(posedge clk); screen_display();
        send_pulse(DASH_LIM); send_gap(SHORT_GAP_LIM); send_pulse(DASH_LIM); send_gap(SHORT_GAP_LIM); send_pulse(DASH_LIM); send_gap(LONG_GAP_LIM); // O
        repeat(50) @(posedge clk); screen_display();
